from utils.imports import cv, np, Image, ImageEnhance
...
def adjust_saturation(image):
    """Prompt the user for a saturation factor and apply it."""
    ...
    if factor:
        ...
        enhanced = ImageEnhance.Color(image_pil).enhance(factor)
        new_image = cv.cvtColor(np.array(enhanced), cv.COLOR_RGB2BGR)
    return new_image


def adjust_saturation_with_factor(image, factor: float):
    """Apply saturation directly with a given factor (used by sliders)."""
